...
def parse_args():
    ...
    parser.add_argument('--sta-weights-path', type=str, default='weights/ppo_jumper_sta') # sta agents
...
def main(args):
    ...
    weights = torch.load(args.sta_weights_path + '.pth')
    ...
    while (len(failures) < 2000):
        ...
        if verbose:
            print(f'Epochs: {len(failures)}, Time Step: {t}, Outcome: {failures[-1]}, Failure Rate: {sum(failures) / len(failures)}')
